const getRandomWord = (wordList) => {
    ...
    const totalWeight = wordList.slice(1).reduce((sum, word) => sum + Number(word[2]), 0);
    ...
    for (let i = 1; i < wordList.length; i++) {
      ...
      if (randomValue <= cumulativeWeight) {
        ...
        return { index: i, word: wordList[i][0], description: wordList[i][1], relevance: wordList[i][2] };
      }
    }
  };
